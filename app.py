...
if "is_admin" not in st.session_state: st.session_state.is_admin = True # จำลอง Admin

# --- 2. ระบบค้นหา Model อัจฉริยะ (Dynamic Model Hunter) ---
# ดึง API KEY จาก Secrets เท่านั้น (ปลอดภัย 100%)
API_KEY = st.secrets.get("GEMINI_API_KEY", "")

def init_ai_engine():
    if not API_KEY:
        return None, "OFFLINE (NO API KEY)"
    try:
        ...
        try:
            ...
            # รายชื่อสำรองรวมถึงรุ่นล่าสุดที่คุณต้องการ
        ...
        # 1. Privilege Logic (Admin vs User)
        if st.session_state.is_admin or st.session_state.usage_count == 0:
            target_list = raw_models 
        else:
            target_list = raw_models[::-1] # วนรุ่นเก่าเพื่อประหยัด

        # Loop & Try: วนหาตัวที่ Online จริง
        for m_name in target_list:
            try:
                ...
                continue
        return None, "OFFLINE (ALL MODELS BUSY)"
    ...
    if bp or site:
        if not st.session_state.engine:
            ...
            with st.spinner("ANALYZING..."):
                try:
                    prompt = 'วิเคราะห์ภาพเทียบกันและตอบเป็น JSON ภาษาไทย: {"analysis":[], "risk":[], "checklist":[], "standard":[], "next_task":"", "materials":[], "owner_note":[]}'
...
if st.session_state.json_data:
    ...
    # Smart Material List: 5-7 รายการ (Text Link เท่านั้น)
    if "materials" in d:
