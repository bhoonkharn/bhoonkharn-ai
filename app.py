...
# --- 2. ENGINE (แม่แบบประหยัด2: บัญชีดำ 2.5/Preview และเน้นรุ่น Quota เยอะ) ---
def init_ai_engine():
    ...
    try:
        ...
        # กรองเฉพาะโมเดลที่มี Vision และไม่ใช่รุ่นโควตาน้อย
        all_m = [m for m in genai.list_models() 
                 if 'generateContent' in m.supported_generation_methods 
                 and 'gemini' in m.name.lower()
                 and '2.5' not in m.name.lower()
                 and 'preview' not in m.name.lower()]
        
        # ลำดับความสำคัญ: 1.5-Flash > 1.5-Pro > Others
...
st.markdown("<div class='story-text'>วิเคราะห์หน้างานก่อสร้างและวางแผนวัสดุล่วงหน้าด้วย AI Vision (แม่แบบประหยัด2)</div>", unsafe_allow_html=True)
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์ข้อมูลและตรวจสอบวัสดุ..."):
        try:
            prompt = f"""ในฐานะ BHOON KHARN AI โหมด {mode} ให้วิเคราะห์ภาพก่อสร้างนี้:
            ระบุข้อมูลจริงในทุกหัวข้อ ห้ามใส่ข้อความ Placeholder:
            ...
            8. [COMPARE] ชื่อวัสดุหลักที่จะเทียบราคา (1 ชื่อสั้นๆ เช่น ปูนเสือ, เหล็กเส้น)
...
if st.session_state.rep:
    ...
    # เปรียบเทียบราคา (จุดที่แก้ไข: ดึง COMPARE ให้แม่นยำขึ้น)
    comp_val = re.search(r"\[COMPARE\]\s*(.*?)(?=\[|$)", st.session_state.rep, re.IGNORECASE)
    if not comp_val: comp_val = re.search(r"COMPARE:\s*(.*?)(?=\[|$)", st.session_state.rep, re.IGNORECASE)
    
    if comp_val:
        render_comparison_grid(comp_val.group(1).strip())
