...
# --- 1. CONFIG & CSS (ยึดตามดีไซน์น้ำตาลทองเดิม) ---
...
# --- 2. PLUGIN: ระบบเปรียบเทียบ (เพิ่มต่อยอด ไม่แก้หลัก) ---
...
# --- 3. CORE ENGINE (ใช้ชื่อตัวแปรเดิม ENGINE 100%) ---
def init_ai_engine():
...
st.markdown("<div class='story-text'>วิเคราะห์งานก่อสร้างด้วยระบบ AI วิศวกรรม คัดสรรวัสดุมาตรฐานจากร้านค้าทางการเพื่อความมั่นใจสูงสุด</div>", unsafe_allow_html=True)

# --- 5. UPLOAD & PROCESSING (ใช้ตรรกะเดิม) ---
c1, c2 = st.columns(2)
with c1: 
...
if st.button("🚀 เริ่มการวิเคราะห์อัจฉริยะ", use_container_width=True, type="primary"):
    if not (bp or site): st.warning("กรุณาอัปโหลดรูปภาพ")
        ...
        with st.spinner("Analyzing..."):
            try:
                # Prompt เดิม เพิ่มแค่การสกัดชื่อวัสดุ
                prompt = f"วิเคราะห์จากภาพโหมด {mode} หัวข้อ: [ANALYSIS], [RISK], [CHECKLIST], [STANDARD], [OWNER_NOTE] และสรุปชื่อวัสดุหลักท้ายรายงานในรูปแบบ [MATERIAL:ชื่อวัสดุ]"
                inps = [prompt]
                if bp: 
