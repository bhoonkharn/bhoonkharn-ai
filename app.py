...
# --- 1. CONFIG & STYLE (แก้ไข UI ตามข้อ 1, 2, 4) ---
st.set_page_config(
...
<style>
    ...
    /* แก้ไขส่วนหัวข้อ (ลบกรอบส่วนเกิน) */
    ...
    /* ข้อ 1: ตกแต่งช่องอัปโหลดโดยตรง ไม่สร้างกล่องเปล่า */
    ...
    /* ข้อ 2: ปุ่มเปลี่ยนภาษาบรรทัดเดียว (Fixed Width & No Wrap) */
    ...
    /* ข้อ 4: จัดราคาวัสดุแบบซ้าย-ขวา (Flexbox) */
    .mat-item { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding: 12px 0; border-bottom: 1px solid rgba(181, 148, 115, 0.1); }
    .mat-link { color: var(--bk-gold); text-decoration: none; font-weight: 700; font-size: 1.05rem; flex-grow: 1; }
    .mat-price { color: #28a745; font-size: 1rem; font-weight: 700; min-width: 80px; text-align: right; }
...
# --- 2. LOGIN LOGIC (แก้ไขข้อ 3: แก้ไขลำดับการเช็คสิทธิ์เพื่อป้องกันการวนลูป) ---
...
# ฟังก์ชันหน้า Login (จะทำงานเฉพาะเมื่อจำเป็นเท่านั้น)
def show_login_gate():
...
if "auth" in st.session_state and "user_info" not in st.session_state:
    try:
        ...
        st.session_state["user_info"] = {"email": "user", "name": "User"}

# ระบบค้นหาโมเดล (ใช้รุ่นใหม่ล่าสุดเสมอสำหรับแอดมิน/สมาชิก)
def get_engine():
    try:
...
with nav_c3:
    with st.popover("👤"):
        # ข้อ 6: กู้คืนเมนูเครื่องมือ Tools
        if "auth" in st.session_state:
            ...
            st.markdown("**GUEST**")
            if st.button("เข้าสู่ระบบ", use_container_width=True): show_login_gate()
...
if st.button("RUN ANALYSIS / เริ่มการวิเคราะห์", use_container_width=True, type="primary"):
    # ตรวจสอบสิทธิ์ (Gatekeeper Logic)
    if "auth" not in st.session_state and st.session_state.usage_count >= 1:
        show_login_gate()
    elif bp or site:
        with st.spinner("AI กำลังวิเคราะห์..."):
            try:
                # ข้อ 5: บังคับส่งรายละเอียดแบบรายการย่อย (List)
                ...
                ใน smart_materials ต้องส่งมา 5-7 รายการ และงานต้องเป็นรายการย่อย 3-5 ข้อเสมอ"""
...
if st.session_state.json_data:
    ...
    # ข้อ 5: แสดงรายการย่อยของงานปัจจุบันและงานถัดไป
    ...
    # ข้อ 4: แสดงราคาวัสดุแบบมืออาชีพ (ซ้าย-ขวา)
...
<div class='footer-box'>
    ...
    <div style='font-size:0.7rem; color:#666; max-width:800px; margin:15px auto; line-height:1.5;'>
        ...
        กรุณาปรึกษาผู้เชี่ยวชาญก่อนดำเนินการใดๆ | Version 2.4 Final 1.3
    </div>
</div>
""", unsafe_allow_html=True)
