...
# --- 2. ENGINE (แม่แบบประหยัด: กรองเฉพาะโมเดลที่มีดวงตา Vision เท่านั้น) ---
def init_ai_engine():
    ...
    try:
        ...
        # แก้ไข: กรองเฉพาะโมเดลตระกูล gemini ที่รองรับการวิเคราะห์ภาพ (Vision) เท่านั้น
        all_m = [m for m in genai.list_models() 
                 if 'generateContent' in m.supported_generation_methods 
                 and 'gemini' in m.name.lower()] # ดีด Gemma ออกไป
        
        # ลำดับความสำคัญ: Flash (ประหยัด/เก่งภาพ) > Pro > Others
        all_m.sort(key=lambda x: ("flash" not in x.name.lower(), "pro" in x.name.lower()))
...
st.markdown("<div class='story-text'>วิเคราะห์งานก่อสร้างและวางแผนวัสดุล่วงหน้าด้วย AI Vision (โหมดเสถียร 2026)</div>", unsafe_allow_html=True)
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์ข้อมูลและวัสดุ..."):
        try:
            # ปรับ Prompt ให้ระบุ Tag ชัดเจน ป้องกันข้อมูลหาย
            prompt = f"""ในฐานะ BHOON KHARN AI โหมด {mode} ให้วิเคราะห์ภาพก่อสร้างนี้:
            1. [ANALYSIS] สรุปหน้างานปัจจุบัน
            2. [RISK] จุดวิกฤตที่ต้องระวัง
            3. [CHECKLIST] เทคนิคการตรวจงานแบบละเอียด
            4. [STANDARD] มาตรฐานวิศวกรรมที่ใช้
            5. [NEXT_TASK] งานลำดับถัดไปที่ต้องทำต่อจากจุดนี้ (ระบุข้อมูลจริง)
            ...
            9. [OWNER_NOTE] แนะนำเจ้าของบ้าน (แยกเป็นข้อๆ เพื่อให้อ่านง่าย)"""
            ...
            # ฟังก์ชันดึงข้อมูลแบบ Fuzzy เพื่อกัน Tag หาย
            def get_data(tag):
...
if st.session_state.rep:
    st.divider()
    # ดึงข้อมูลมาแสดงผล
    def display_sec(title, tag):
        content = re.search(f"\[{tag}\]\s*(.*?)(?=\[|$)", st.session_state.rep, re.DOTALL | re.IGNORECASE)
        if content:
            st.markdown(f"<div class='section-header'>{title}</div>", unsafe_allow_html=True)
            st.write(content.group(1).strip())

    display_sec("🔍 วิเคราะห์หน้างาน", "ANALYSIS")
    display_sec("⚠️ จุดวิกฤต", "RISK")
    display_sec("📝 เทคนิคการตรวจ", "CHECKLIST")
    display_sec("🏗️ มาตรฐานวิศวกรรม", "STANDARD")
