...
# --- 1. CONFIG & STYLE (ยึดตามแม่แบบ 3 แต่ขยายขนาดรูป) ---
...
<style>
    ...
    .btn-search-v3 { color: var(--bk-gold) !important; text-decoration: none; border: 1px solid var(--bk-gold); padding: 8px 18px; border-radius: 8px; font-size: 0.9rem; font-weight: bold; }
    .btn-search-v3:hover { background: var(--bk-gold); color: white !important; }
...
# --- 2. ENGINE (ใช้ Engine เดิมของแม่แบบ 3 ที่เสถียรที่สุด) ---
def init_ai_engine():
    ...
    try:
        ...
        # จัดลำดับความสำคัญให้ 1.5 Flash
        def m_rank(n):
            n = n.lower()
            if "2.5" in n or "3" in n: return 99
...
# --- 3. UI FUNCTIONS (ปรับเป็น 5 คอลัมน์ รูปใหญ่) ---
def render_shopping_list_v3(materials_data):
    ...
    st.markdown("""
        <div class='mat-table-header'>
            ...
            <div style='flex:3; padding-left:30px;'>สเปกวัสดุที่แนะนำ</div>
    ...
    for i, item in enumerate(materials_data):
        ...
        with cols[2]: st.markdown(f"<div style='margin-top:45px; font-weight:700; font-size:1.15rem;'>{name}</div><div style='color:#A09080; font-size:0.9rem;'>รุ่นมาตรฐานงานก่อสร้างไทย</div>", unsafe_allow_html=True)
        ...
        with cols[4]: st.markdown(f"<div style='margin-top:50px; text-align:right;'><a href='https://www.google.com/search?q={name}+ราคา' target='_blank' class='btn-search-v3'>🌐 คลิก</a></div>", unsafe_allow_html=True)
...
st.markdown("<div class='story-text'>วิเคราะห์หน้างานด้วย AI (Stable V3 Visual XL)</div>", unsafe_allow_html=True)
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์..."):
        try:
            # สั่งให้ตอบเป็น List เพื่อแยกข้อ
...
if st.session_state.json_data:
    ...
    if "future_materials" in d:
        render_shopping_list_v3(d["future_materials"])

    if "owner_note" in d:
        st.markdown("<div class='section-header'>🏠 แนะนำสำหรับเจ้าของบ้าน</div>", unsafe_allow_html=True)
        items = d["owner_note"]
        if isinstance(items, list):
