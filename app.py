...
<style>
    ...
    .mat-thumb { width: 55px; height: 55px; border-radius: 6px; object-fit: cover; border: 1px solid rgba(181, 148, 115, 0.3); }
...
# --- 2. ENGINE (Force JSON Mode for Lite/Stable Models) ---
def init_ai_engine():
    ...
    try:
        ...
        all_m = [m for m in genai.list_models() if 'gemini' in m.name.lower()]
        # ลำดับความสำคัญ: 1.5 Flash (โควตาเยอะ) > Flash-Lite > Others
        all_m.sort(key=lambda x: ("1.5-flash" not in x.name.lower(), "lite" not in x.name.lower()))
...
def render_visual_shopping_list(materials):
    ...
    for i, mat in enumerate(materials):
        # ล้างอักขระพิเศษเพื่อทำ Mapping รูป
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์และจัดทำตารางวัสดุ..."):
        try:
            # ใช้ JSON Schema เพื่อบังคับให้ Lite Model ตอบอย่างมีระเบียบ
            prompt = f"""ในฐานะ BHOON KHARN AI โหมด {mode} ให้วิเคราะห์ภาพและตอบในรูปแบบ JSON เท่านั้น:
            {{
                "analysis": "สรุปหน้างาน",
                "risk": "ความเสี่ยง",
                "checklist": "เทคนิคการตรวจงาน",
                "standard": "มาตรฐานวิศวกรรม",
                "next_task": "งานลำดับถัดไป (ระบุข้อมูลจริง)",
                "future_materials": ["ชื่อวัสดุ 1", "ชื่อวัสดุ 2", "ชื่อวัสดุ 3", "ชื่อวัสดุ 4"],
                "owner_note": "แนะนำเจ้าของบ้าน"
            }}
            ห้ามใส่ข้อความ Placeholder ห้ามใส่ Markdown Code Block ให้ตอบเฉพาะเนื้อหา JSON"""
...
if st.session_state.json_data:
    ...
    # ตารางวัสดุ (Visual Shopping List)
