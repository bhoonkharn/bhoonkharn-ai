...
# --- 2. DYNAMIC ENGINE (ค้นหาและเลือกตัวที่ดีที่สุด/ใหม่ที่สุดจาก List จริง) ---
def init_ai_engine():
    ...
    try:
        ...
        # ขั้นตอนที่ 1: ดึงรายชื่อโมเดลทั้งหมดที่ API Key ของคุณเข้าถึงได้
        # และต้องรองรับการวิเคราะห์รูปภาพ (generateContent)
        all_models = [m for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        
        # ขั้นตอนที่ 2: จัดลำดับความสำคัญ (Pro > Flash และ เวอร์ชั่นใหม่ > เวอร์ชั่นเก่า)
        # เราจะจัดเรียงโดยดูจาก Keyword ในชื่อโมเดล
        sorted_models = sorted(all_models, key=lambda x: (
            "2.0" in x.name,   # ให้เวอร์ชั่น 2.0 (ถ้ามี) มาก่อน
            "pro" in x.name,   # ให้รุ่น Pro มาก่อน Flash
            "1.5" in x.name    # ให้รุ่น 1.5 มาก่อน 1.0
        ), reverse=True)

        # ขั้นตอนที่ 3: วนลูปทดสอบเพื่อหาตัวที่ "ใช้งานได้จริง" และไม่ติด 404
        for m in sorted_models:
            try:
                # ลองเรียกใช้โมเดลตัวที่ฉลาดที่สุดในลิสต์ก่อน
                model = genai.GenerativeModel(m.name)
                # ทดสอบเรียกสั้นๆ เพื่อยืนยันว่าไม่ติด Error 404
                model.generate_content("hi", generation_config={"max_output_tokens": 1})
                return model, f"เชื่อมต่อสำเร็จ: {m.name.split('/')[-1]}"
            except:
                continue # ถ้าตัวที่ฉลาดที่สุดติดปัญหา ให้ลองตัวถัดไปในลิสต์ทันที
                
        return None, "ไม่พบโมเดลที่รองรับในระบบของคุณ"
...
def render_comparison(material_name):
    ...
    with c_b: st.markdown("**🏆 เกรดพรีเมียม:** คุณสมบัติสูงกว่า (เช่น ทนทานพิเศษ) อายุการใช้งานยาวนานกว่า")
...
# เพิ่ม Story Text (ตามที่ตกลงกันไว้)
...
if st.button("🚀 เริ่มการวิเคราะห์อัจฉริยะ", use_container_width=True, type="primary"):
    if not (bp or site):
        st.warning("กรุณาอัปโหลดรูปภาพก่อนวิเคราะห์")
    else:
        with st.spinner("AI กำลังเลือกโมเดลและวิเคราะห์ข้อมูล..."):
            try:
                # ปรับ Prompt เพื่อสกัดชื่อวัสดุ
...
if st.session_state.rep:
    st.divider()
    sections = [("🔍 สรุปผลการวิเคราะห์", "[ANALYSIS]"), ("⚠️ จุดวิกฤต", "[RISK]"), ("📝 เทคนิคการตรวจงาน", "[CHECKLIST]"), ("🏗️ มาตรฐานวิศวกรรม", "[STANDARD]"), ("🏠 แนะนำเจ้าของบ้าน", "[OWNER_NOTE]")]
    ...
    # ดึงชื่อวัสดุมาเข้าระบบเปรียบเทียบ (Plugin ใหม่)
