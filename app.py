...
# --- 2. ENGINE (แม่แบบประหยัด: ล็อครุ่น 1.5 Flash เพื่อโควตาสูงสุด และกัน Error 429/400) ---
def init_ai_engine():
    ...
    try:
        ...
        # กลยุทธ์ประหยัดโควตา: เรียงลำดับจากรุ่นเสถียร (1.5) ไปรุ่นใหม่ (2.5) เพื่อเลี่ยงโควตาเต็ม
        def quota_priority(m_name):
            name = m_name.lower()
            if "1.5-flash" in name: return 1  # ลำดับ 1: โควตาเยอะสุด (ประมาณ 1,500/วัน)
            if "1.5-pro" in name: return 2    # ลำดับ 2: โควตาปานกลาง
            if "2.0" in name: return 3        # ลำดับ 3: รุ่นใหม่โควตาเริ่มน้อย
            if "2.5" in name: return 10       # ลำดับสุดท้าย: รุ่นทดสอบโควตาน้อยมาก (20/วัน)
            return 5

        all_m.sort(key=lambda x: quota_priority(x.name))

        for m_info in all_m:
            try:
                model = genai.GenerativeModel(m_info.name)
                model.generate_content("test", generation_config={"max_output_tokens": 1})
                return model, "Online" # แสดงแค่ Online ตามสั่ง
            except: continue
        return None, "Offline"
...
with st.sidebar:
    ...
    st.markdown(f"Status: **{st.session_state.status}**") # แสดงแค่ Online/Offline
...
st.markdown("<div class='story-text'>วิเคราะห์งานก่อสร้างด้วย AI Vision ประสิทธิภาพสูง พร้อมวางแผนวัสดุล่วงหน้า (Economy Mode)</div>", unsafe_allow_html=True)
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์หน้างานและคำนวณวัสดุ..."):
        try:
            # ปรับ Prompt ให้ระบุหัวข้อชัดเจน ป้องกันข้อมูลหาย
            prompt = f"""วิเคราะห์ภาพก่อสร้างนี้ในโหมด {mode}:
            หัวข้อที่ต้องระบุข้อมูลจริง (ห้ามเว้นว่าง):
            1. [ANALYSIS] สรุปหน้างานปัจจุบัน
            2. [RISK] จุดวิกฤตที่ต้องระวัง
            3. [CHECKLIST] เทคนิคการตรวจงานโดยละเอียด
            4. [STANDARD] มาตรฐานวิศวกรรมที่เกี่ยวข้อง
            5. [NEXT_TASK] ระบุงานที่ต้องทำต่อจากในรูปนี้ทันที (เขียนมา 1-2 บรรทัด)
            ...
            9. [OWNER_NOTE] คำแนะนำพิเศษสำหรับเจ้าของบ้าน (แยกเป็นข้อๆ)"""
            ...
            # ระบบดึงข้อมูลแบบ Fuzzy Regex ป้องกัน Tag หาย (ข้อ 3, 4, 5)
            def get_val(tag):
                pattern = f"\[{tag}\]\s*(.*?)(?=\[|$)"
                match = re.search(pattern, res.text, re.DOTALL | re.IGNORECASE)
                if not match:
                    pattern = f"{tag}:\s*(.*?)(?=\[|$)"
                    match = re.search(pattern, res.text, re.DOTALL | re.IGNORECASE)
                return match.group(1).strip() if match else ""

            st.session_state.next_task = get_val("NEXT_TASK")
            st.session_state.materials = [x.strip() for x in get_val("MATERIALS").split(",") if x.strip()]
            st.session_state.next_materials = [x.strip() for x in get_val("NEXT_MATERIALS").split(",") if x.strip()]
...
if st.session_state.rep:
    st.divider()
    def draw_sec(title, tag):
        content = re.search(f"\[{tag}\]\s*(.*?)(?=\[|$)", st.session_state.rep, re.DOTALL | re.IGNORECASE)
        if content:
            st.markdown(f"<div class='section-header'>{title}</div>", unsafe_allow_html=True)
            st.write(content.group(1).strip())

    draw_sec("🔍 วิเคราะห์หน้างาน", "ANALYSIS")
    draw_sec("⚠️ จุดวิกฤต", "RISK")
    draw_sec("📝 เทคนิคการตรวจ", "CHECKLIST")
    draw_sec("🏗️ มาตรฐานวิศวกรรม", "STANDARD")
