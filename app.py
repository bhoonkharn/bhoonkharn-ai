...
# --- 2. ENGINE (แม่แบบประหยัด: ล็อคเป้า 1.5-Flash และแบนรุ่นโควตาน้อย 2.5/Preview) ---
def init_ai_engine():
    ...
    try:
        ...
        # กรองเฉพาะโมเดล Gemini ที่มี Vision และไม่ใช่รุ่นทดลองที่มีโควตาน้อย
        all_m = [m for m in genai.list_models() 
                 if 'generateContent' in m.supported_generation_methods 
                 and 'gemini' in m.name.lower()
                 and '2.5' not in m.name.lower() # บัญชีดำรุ่น 2.5
                 and 'preview' not in m.name.lower() # บัญชีดำรุ่น Preview
                 and 'robotics' not in m.name.lower()] # บัญชีดำรุ่นหุ่นยนต์
        
        # ลำดับความสำคัญ: 1.5-Flash (โควตาเยอะสุด) > 1.5-Pro > Others
...
def render_comparison_grid(material_name):
    ...
    # ระบบจับคู่รูปภาพอัจฉริยะ (Fallback System)
    img_db = {
...
st.markdown("<div class='story-text'>วิเคราะห์งานก่อสร้างและวางแผนงานล่วงหน้าด้วย AI Vision (Economical Template 2026)</div>", unsafe_allow_html=True)
...
def run_analysis():
    ...
    with st.spinner("AI กำลังวิเคราะห์และคำนวณงานลำดับถัดไป..."):
        try:
            prompt = f"""ในฐานะ BHOON KHARN AI โหมด {mode} ให้วิเคราะห์ภาพก่อสร้างนี้และตอบในรูปแบบที่กำหนด:
            1. [ANALYSIS] สรุปสภาพหน้างานปัจจุบัน
            2. [RISK] จุดวิกฤตหรือความเสี่ยงที่ต้องระวัง
            3. [CHECKLIST] เทคนิคการตรวจงานโดยละเอียด (แยกเป็นข้อๆ)
            4. [STANDARD] มาตรฐานวิศวกรรมที่เกี่ยวข้อง
            5. [NEXT_TASK] งานลำดับถัดไปที่ต้องทำต่อจากจุดนี้ (ระบุข้อมูลจริง 1-2 บรรทัด)
            6. [MATERIALS] รายการวัสดุจุกจิกที่ต้องใช้ตอนนี้ (คั่นด้วยคอมม่า)
            7. [NEXT_MATERIALS] รายการวัสดุที่ต้องเตรียมสำหรับงานถัดไป (คั่นด้วยคอมม่า)
            8. [COMPARE] ชื่อวัสดุหลักที่จะเทียบราคา (1 ชื่อสั้นๆ)
            9. [OWNER_NOTE] คำแนะนำพิเศษสำหรับเจ้าของบ้าน (แยกบรรทัดอ่านง่าย)"""
            ...
            # ระบบสกัดข้อมูลแบบ Fuzzy Regex (ดักทุกรูปแบบ Tag)
            def get_data(tag):
                m = re.search(f"\[{tag}\]\s*(.*?)(?=\[|$)", res.text, re.DOTALL | re.IGNORECASE)
                if not m: m = re.search(f"{tag}:\s*(.*?)(?=\[|$)", res.text, re.DOTALL | re.IGNORECASE)
...
if st.session_state.rep:
    ...
    # 4. เปรียบเทียบราคา (หัวข้อสะอาดตา)
    ...
    # 5. คำแนะนำเจ้าของบ้าน (อ่านง่ายขึ้น)
    ...
    # 6. ส่วนติดต่อและหมายเหตุ (Contact & Disclaimer)
    st.markdown(f"""
