...
# CSS: เน้นความสะอาดตา และสีแดงเลือดหมูสำหรับหมายเหตุ
st.markdown("""
    <style>
    .disclaimer-text {
        color: #8B0000; 
    ...
    if not st.session_state.engine: st.error("AI ไม่พร้อมใช้งาน")
        ...
        with st.spinner('BHOON KHARN AI กำลังประมวลผล...'):
            try:
                ...
                (สรุปเป็นข้อๆ โดยใช้ Emoji นำหน้า และต้องขึ้นบรรทัดใหม่ 2 ครั้งทุกครั้งที่จบข้อ)
...
# --- 6. ส่วนการแสดงผลรายงาน (ปรับปรุงระบบแยกหัวข้อให้ยืดหยุ่นขึ้น) ---
if st.session_state.final_report:
    st.divider()
    st.markdown("### 📋 ผลการตรวจสอบและวิเคราะห์")
    
    text = st.session_state.final_report
    # ใช้ Regex ที่ยืดหยุ่นขึ้น (
